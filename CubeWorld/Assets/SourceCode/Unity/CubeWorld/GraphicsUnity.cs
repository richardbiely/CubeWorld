...
public class GraphicsUnity
{
    ...
    public static Vector3 CubeWorldVector3ToVector3(CubeWorld.Utils.Vector3 vec)
    {
        return new Vector3(vec.x, vec.y, vec.z);
    }

    public static CubeWorld.Utils.Vector3 Vector3ToCubeWorldVector3(Vector3 vec)
    {
        return new CubeWorld.Utils.Vector3(vec.x, vec.y, vec.z);
    }

    public static Color CubeWorldColorToColor(CubeWorld.Utils.Color cubeWorldColor)
    {
        return new Color(cubeWorldColor.r, cubeWorldColor.g, cubeWorldColor.b, cubeWorldColor.a);
    }

    public static Texture2D GetTilesetTexture(Texture2D tilesetTexture, int tileNumber)
    {
        int uvdelta = tilesetTexture.width / TILE_PER_MATERIAL_ROW;

        int uvx = uvdelta * (tileNumber % TILE_PER_MATERIAL_ROW);
        int uvy = tilesetTexture.height - uvdelta * (tileNumber / TILE_PER_MATERIAL_ROW);

        return GetTextureSlice(tilesetTexture, uvx, uvy, uvdelta, uvdelta);
    }

    public static Texture2D GetTextureSlice(Texture2D from, int x, int y, int width, int height)
    {
